...
Future<void> firebaseMessagingBackgroundHandler(RemoteMessage message) async {
  ...
  debugPrint(
    '[NotificationService] Notification: ${message.notification != null ? "YES" : "NO"}',
  );
...
class NotificationService {
  ...
  static const AndroidNotificationChannel _channel = AndroidNotificationChannel(
    ...
    description:
        'This channel is used for important notifications with sound and vibration.',
  ...
  Future<void> initialize() async {
    ...
    if (_isInitialized) {
      debugPrint(
        '[NotificationService] ⚠️ Already initialized - skipping re-initialization',
      );
      return;
    }
    ...
    try {
      ...
      debugPrint(
        '[NotificationService] Step 4: Setting up message handlers...',
      );
      _setupMessageHandlers();

      // Setup token refresh listener
      debugPrint(
        '[NotificationService] Step 5: Setting up token refresh listener...',
      );
  ...
  Future<void> _requestPermissions() async {
    if (Platform.isIOS) {
      ...
      if (await Permission.notification.isDenied) {
        ...
        debugPrint(
          '[NotificationService] Android 13+ notification permission: $status',
        );

        if (status.isPermanentlyDenied) {
          debugPrint(
            '[NotificationService] ⚠️ Notification permission permanently denied. User needs to enable in settings.',
          );
        }
      }
      ...
      debugPrint(
        '[NotificationService] Android permissions requested and channel created',
      );
    }
  }
  ...
  Future<void> _handleForegroundMessage(RemoteMessage message) async {
    ...
    if (message.messageId != null &&
      ...
      debugPrint(
        '[NotificationService] ⚠️ Duplicate message detected - already processed: ${message.messageId}',
      );
      return;
    }
    ...
    if (message.messageId != null) {
      ...
      debugPrint(
        '[NotificationService] ✓ Message ID added to processed list (${_processedMessageIds.length} total)',
      );
    }
    ...
    if (!_isUserLoggedIn) {
      debugPrint(
        '[NotificationService] ⚠️ User not logged in - skipping notification',
      );
      return;
    }
    ...
    if (message.notification != null) {
      ...
      debugPrint(
        '[NotificationService] ⚠️ No notification payload - skipping local notification',
      );
    }
  }
  ...
  Future<void> _checkInitialMessage() async {
    ...
    if (initialMessage != null) {
      ...
      debugPrint(
        '[NotificationService] Message ID: ${initialMessage.messageId}',
      );
      _handleNotificationTap(initialMessage);
    } else {
      debugPrint(
        '[NotificationService] No initial message (normal app launch)',
      );
    }
  }
  ...
  Future<void> _showLocalNotification({
    ...
    try {
      ...
      const iosDetails = DarwinNotificationDetails(
        ...
        interruptionLevel:
            InterruptionLevel.active, // Active interruption level for sound
      );
      ...
      debugPrint(
        '[NotificationService] ❌ Error showing local notification: $e',
      );
    }
  }
  ...
  void _navigateBasedOnData(Map<String, dynamic> data) {
    ...
    if (data.containsKey('type')) {
      switch (data['type']) {
        ...
        default:
          debugPrint(
            '[NotificationService] Unknown notification type: ${data['type']}',
          );
      }
    }
  }
  ...
  void _startMessageIdCleanupTimer() {
    ...
    _messageIdCleanupTimer = Timer.periodic(const Duration(minutes: 5), (
      timer,
    ) {
      final oldCount = _processedMessageIds.length;
      _processedMessageIds.clear();
      debugPrint(
        '[NotificationService] 🧹 Cleaned up processed message IDs (removed $oldCount)',
      );
    });
  }
  ...
  void enableNotifications() {
    ...
    debugPrint(
      '[NotificationService] ✅ Notifications ENABLED - User logged in',
    );
  }
  ...
  Future<void> disableNotifications() async {
    ...
    debugPrint(
      '[NotificationService] 🔕 Notifications DISABLED - User logged out',
    );
